...
final class PluginBootstrap {
    ...
    static def provide() {
        List<Class<?>> listeners = init "./target/classes/plugin/"
        ...
        Files.walkFileTree(Paths.get(dir),
                new SimpleFileVisitor<Path>() {
                    ...
                    FileVisitResult visitFile(Path path, BasicFileAttributes attrs) {
                        ...
                        String formatted = file.path.replace(File.separator, ".").substring(0, file.path.lastIndexOf(".")).replace("target.", "").replace("classes.", "").replace("..", "")
